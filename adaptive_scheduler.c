...
int s_counter = 9;
int low_threshold = 7;
int high_threshold = 12;
int flag = 0;
int start = -1;
dram_address_t address;
int counter = 1;
...
void schedule(int channel)
{
	...
	int i, j;
        ...
        if(s_counter > high_threshold && counter == 1 ){
           flag = 1;
           counter = 0;
        }
        else if(s_counter < low_threshold && counter == 0){
             flag = 0;
             counter = 1;
        }        
			...
			if(wr_ptr->command_issuable)
			{
				...
				if (wr_ptr->next_command == PRE_CMD) {
				  recent_colacc[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank] = 0;
                                ...
                                else if(start != -1 && wr_ptr->dram_addr.actual_address == address.actual_address && wr_ptr->dram_addr.channel == address.channel && wr_ptr->dram_addr.rank == address.rank && wr_ptr->dram_addr.bank == address.bank && wr_ptr->dram_addr.row == address.row){
                                   s_counter--;
                                }
                                }
                                   address = wr_ptr->dram_addr;
                                else {
                                if(start == -1){
                                   start = 1
                                }
			...
			if(rd_ptr->command_issuable)
			{
				...
				if (rd_ptr->next_command == PRE_CMD) {
				  recent_colacc[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] = 0;
                                ...
                                else if(start != -1 && rd_ptr->dram_addr.actual_address == address.actual_address && rd_ptr->dram_addr.channel == address.channel && rd_ptr->dram_addr.rank == address.rank && rd_ptr->dram_addr.bank == address.bank && rd_ptr->dram_addr.row == address.row){
                                   s_counter--;
                                }
                                 address = rd_ptr->dram_addr;
                                }
